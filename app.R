...
#parameters used in game initiation
def_number_of_nations <- 5      #number between 1 and 12
def_number_of_empty_cells <- 12       #advised no more than 300
def_number_of_free_cities <- 8       
def_base_number_of_cores <- 3   
def_tiles_iterations <- 10      
def_names_territories_list <- unlist(unname(read.csv("data/nomi comuni italiani.csv")))  #check list is long enough
def_names_nations_list <- unlist(unname(read.csv("data/nomi inventati.csv")))    #check list is long enough
def_n_control_nations <- 1       #number of nations controlled 0,...,number_of nations
def_player_mode <- "shared screen"   #"shared screen" to introduce turns between players in inserting orders
def_central_handicap <- TRUE
def_random_assignment <- FALSE
def_turns_core_acquisition <- 6
def_type_ai <- "mixed"  # base, ML, mixed
def_n_tiles_troops <- 3 #number of empty tiles allowing recruitment 1 troop
def_colour_palette <- c("#e4201b", "#377eb8", "#4caf4a", "#984ea3", "#ff7f00", "#fffc33", "#a65628", "#f781bf", "#999999", "#a5cee3", "#fb9a99", "#b2df89")
#palette colour order: #red, blue, green, purple, orange, yellow, brown, pink, grey, light blue, light red, light green  

#parameters game
start_year <- 0    #first year in game



# FUNCTIONS --------------------------------

#import map and game generation tools
...
source("function evaluate orders.R")
source("function execute orders.R")
...
ui <- fluidPage(
  ...
  sidebarLayout(position = "left",
                ...
                sidebarPanel(
                  ...
                  #TABS MAP inside LEFT panel -------
                  tabsetPanel(type = "tabs",
                              tabPanel("Game", HTML("<br><br>"),
                                       useShinyjs(),
                                       div(style="text-align: center",
                                           
                                           p(actionButton("recruit_ui","Recruit", width = "150px")),
                                           p(actionButton("move_ui","Move", width = "150px")),
                                           p(actionButton("unstack_ui","Unstack", width = "150px")), 
                                           p(actionButton("disband_ui","Disband", width = "150px")),
                                           p(actionButton("ruler_ui","King", width = "150px")),HTML("<br><br><br>"),
                                           
                                           p(actionButton("clear_select_ui","Click & clear", width = "150px")),
                                           p(actionButton("clear_order_ui","Clear last", width = "150px")),
                                           p(actionButton("clear_all_ui","Clear all", width = "150px")),HTML("<br><br><br>"),
                                           
                                           actionButton("next_turn_ui","Next turn", width = "150px",
                                                        style="color: #fff; background-color: #337ab7; border-color: #2e6da4")
                                       ) #div end
                                       
                              ), #end tab orders/map
                              
                              #TABS MENU inside LEFT panel -------         
                              tabPanel("Menu", 
                                       ...
                                       div(style="text-align: center",
                                           h3("Game options"),
                                           HTML("<br>"),
                                           fileInput("load_ui",label = NULL,accept = ".R", buttonLabel = "Load game"),
                                           downloadButton("save_ui","Save game", width = "200px"),
                                           HTML("<br><br>"),
                                           selectInput("ai_difficulty_ui", h4("AI settings"), 
                                                       choices = list("Select difficulty"=0,"Easy" = 0.8, "Normal" = 1,
                                                                      "Hard" = 1.3, "Very hard" = 1.6,
                                                                      "Extreme" = 2), selected = 0),
                                           htmlOutput(outputId = "difficulty_level"),
                                           
                                           HTML("<br><br><br>"),
                                           h3("Start a new game"),
                                           actionButton("new_game_ui","New game"),
                                           
                                           HTML("<br><br>"),
                                           h4("Customise the new game"),
                                       ), #div end
                                       
                                       
                                       helpText("Here you can customise your new game"),
                                       # Input: Sliders for new game options
                                       sliderInput(inputId = "n_nations_ui",
                                                   label = "Number of nations:",
                                                   min = 2,
                                                   max = 12,
                                                   value = def_number_of_nations),
                                       uiOutput("n_players_ui_slider"),
                                       sliderInput(inputId = "n_base_cells_ui",
                                                   label = "Base territories:",
                                                   min = 1,
                                                   max = 5,
                                                   value = def_base_number_of_cores),
                                       sliderInput(inputId = "n_empty_cells",
                                                   label = "Empty territories:",
                                                   min = 0,
                                                   max = 30,
                                                   value = def_number_of_empty_cells),
                                       sliderInput(inputId = "n_free_cities_ui",
                                                   label = "Empty cities:",
                                                   min = 0,
                                                   max = 30,
                                                   value = def_number_of_free_cities),
                                       sliderInput(inputId = "turns_core_acquisition_ui",
                                                   label = "Turns required for core acquisition:",
                                                   min = 0,
                                                   max = 20,
                                                   value = def_turns_core_acquisition),
                                       sliderInput(inputId = "n_iterations_ui",
                                                   label = "Shape regularity:",
                                                   min = 1,
                                                   max = 30,
                                                   value = def_tiles_iterations),
                                       checkboxInput(inputId = "central_handicap_ui",
                                                     label = "Central handicap",
                                                     value = def_central_handicap),
                                       checkboxInput(inputId = "random_assignment_ui",
                                                     label = "Random assignment",
                                                     value = def_random_assignment)
                                       
                              ) #end tab menu 
                              
                  ), #end tab environment
                  
                  width = 3),  #end sidebar environment
                ...
                mainPanel(
                  ...
                  #TURN NAME:
                  h4(htmlOutput(outputId = "turn_name")),
                  
                  #MAP:
                  plotlyOutput(outputId = "displayed_map", width = "1000px", height = "800px"),
                  
                  #BUTTONS UNDER MAP:
                  div(style="text-align: center",
                      actionButton("show_previous_ui","Show previous", width = "150px"),
                      actionButton("nations_table_ui","Nations table", width = "150px"),
                      actionButton("stats_ui","Stats", width = "150px")),
...
server <- function(input, output) {
  ...
  #----------------------- DYNAMIC SLIDER N PLAYERS ------------
  output$n_players_ui_slider <- renderUI({
    sliderInput("n_players_ui", "Number of player-controlled nations", min = 0,
                max = input$n_nations_ui, value=1,step= 1)
  })
  ...
  showModal(modalDialog(
    ...
    "A new board is being generated for you, this may take a few seconds ...",
    easyClose = TRUE,
    fade =TRUE,
    size= "l"
  ))
  ...
  #---------------------------- CORE ELEMENTS ------------------------
  #Initiate game
  new_game <- initiate_game(number_of_nations = def_number_of_nations,
                            number_of_empty_cells = def_number_of_empty_cells,
                            number_of_free_cities = def_number_of_free_cities,
                            base_number_of_cores = def_base_number_of_cores,
                            tiles_iterations = def_tiles_iterations,
                            names_territories_list = def_names_territories_list,
                            names_nations_list = def_names_nations_list,
                            n_control_nations = min(def_n_control_nations,def_number_of_nations),
                            central_handicap = def_central_handicap,
                            random_assignment = def_random_assignment,
                            colour_palette = def_colour_palette,
                            player_mode = def_player_mode)
  seasons <- new_game$seasons
  
  #make core game items reactive
  game <- reactiveValues(territories = new_game$territories,
                         nations = new_game$nations,
                         turn = start_year,
                         orders = new_game$orders,
                         hidden_orders = new_game$orders,
                         difficulty_bias = 1,
                         player_to_play = 1,
                         stats_turns = cbind("turn"=start_year,new_game$nations[,c("names","colours","tot_tiles","tot_cities","tot_troops")]))
  
  #make interface elements reactive
  interface <- reactiveValues(input_order_state = NA, # "mode" for user input click (e.g. move,recruit,unstack). This is used to understand what order of clicks are expected.
                              clicks = NA,
                              last_click = NA,
                              curr_click = NA,
                              loaded_game = NULL,
                              temp = NA, #temporary dynamic variable
                              previous_results = NULL,
                              var_plot = "tot_cities",
                              turns_core_acquisition = def_turns_core_acquisition)   
  
  #TURN NAME from turn number
  output$turn_name<- renderText(paste0("Year ",game$turn%/%(length(seasons)*2), ", ", seasons[(game$turn %/% 2) %% length(seasons) + 1], ifelse(game$turn %% 2 == 1,
                                                                                                                                                ": <b>RESULTS</b>",
                                                                                                                                                ifelse(def_player_mode=="shared screen",
                                                                                                                                                       paste0(": turn of <span style=\"color:",game$nations$colour[game$nations$is_player][game$player_to_play],"\">",substr(game$nations$names[game$nations$is_player][game$player_to_play],1,nchar(game$nations$names[game$nations$is_player][game$player_to_play])-8),"</span>"),
                                                                                                                                                       ": "))))
  
  #AI difficulty level set and display in menu
  observeEvent(input$ai_difficulty_ui,{
    if (input$ai_difficulty_ui>0){game$difficulty_bias <- input$ai_difficulty_ui}
  })
  output$difficulty_level <- renderText(paste("Current playing: <b>",
                                              switch(as.character(game$difficulty_bias),
                                                     "0.8"="Easy",
                                                     "1"="Normal",
                                                     "1.3"="Hard",
                                                     "1.6"="Very Hard",
  ...
  #MAP
  output$displayed_map<- renderPlotly(  
    if (nrow(game$orders)>0){
      if ((game$turn %/% 2) %% length(seasons) + 1 == length(seasons)){plot_map(territories_data = game$territories, nations_data = game$nations, orders_data = game$orders , core_highlight = TRUE)
      } else {plot_map(territories_data =  game$territories, nations_data = game$nations, orders_data = game$orders, player_to_play = game$player_to_play)}
      
    } else {
      if ((game$turn %/% 2) %% length(seasons) + 1 == length(seasons)){plot_map(territories_data = game$territories,nations_data = game$nations, core_highlight = TRUE)
      } else {plot_map(territories_data = game$territories,nations_data = game$nations, player_to_play= game$player_to_play)}
    })
  
  #SHOW COMMAND BUTTONS FOR TURN 1
  ...
  # FOR BUTTON IN MENU
  observeEvent(input$new_game_ui,{
    
    showNotification("Generating a new game. This may take a few seconds...", type = "message")
    
    #initiate new game and replace old game elements
    interface$loaded_game <- initiate_game(number_of_nations = input$n_nations_ui,
                                           number_of_empty_cells = input$n_empty_cells,
                                           number_of_free_cities = input$n_free_cities_ui,
                                           base_number_of_cores = input$n_base_cells_ui,
                                           tiles_iterations = input$n_iterations_ui,
                                           names_territories_list = def_names_territories_list,
                                           names_nations_list = def_names_nations_list,
                                           n_control_nations = input$n_players_ui,
                                           central_handicap = input$central_handicap_ui,
                                           random_assignment = input$random_assignment_ui,
                                           colour_palette = def_colour_palette,
                                           player_mode = def_player_mode)
    game$territories <- interface$loaded_game$territories
    game$nations <- interface$loaded_game$nations
    game$turn <- start_year
    game$orders <- interface$loaded_game$orders
    game$hidden_orders <- new_game$orders
    game$player_to_play <- 1
    game$stats_turns <- cbind("turn"=start_year,interface$loaded_game$nations[,c("names","colours","tot_tiles","tot_cities","tot_troops")])
  
    interface$previous_results <- NULL
    interface$turns_core_acquisition <- input$turns_core_acquisition_ui
    
    
    #SHOW ONLY BUTTONS OF TURN 1
    shinyjs::showElement(anim=TRUE, animType="fade", time=0.8,id="move_ui")
    shinyjs::showElement(anim=TRUE, animType="fade", time=0.8,id="unstack_ui")
    shinyjs::showElement(anim=TRUE, animType="fade", time=0.8,id="ruler_ui")
    shinyjs::hideElement(anim=TRUE, animType="fade", time=0.8,id="recruit_ui")
    ...
    #SUPPORT SHARES SCREEN MODE ON TURN 1
  ...
  # FOR BUTTON IN WINNER POPUP
  observeEvent(input$won_new_game,{
  ...
  #SAVE BUTTON
  output$save_ui <- downloadHandler(
    filename = function() {
      paste0("Year ",game$turn%/%(length(seasons)*2), " - ", seasons[(game$turn %/% 2) %% length(seasons) + 1]," (",Sys.Date(),").Rdata")
    },
    content = function(file) {
      saved_game <- list(territories=game$territories, nations = game$nations, turn = game$turn, orders = game$orders,difficulty_bias=game$difficulty_bias,player_to_play = game$player_to_play, hidden_orders = game$hidden_orders, previous_results = interface$previous_results, stats_turns = game$stats_turns, turns_core_acquisition = interface$turns_core_acquisition )
      save(saved_game, file = file)
    }
  )
  
  #LOAD BUTTON
  observeEvent(input$load_ui,{
    interface$input_order_state <- NA
    interface$last_click<-NA
    interface$curr_click<-NA
    
    if (!is.null(input$load_ui)){
      #get file path
      temp <- input$load_ui
      
      #create new environment for loading file and import file from the new environment
      load_env <- new.env()
      loaded_game <- load(temp$datapath, envir = load_env)
      interface$loaded_game <- load_env[[loaded_game]]
      
      #update game data
      game$territories <- interface$loaded_game$territories
      game$nations <- interface$loaded_game$nations
      game$turn <- interface$loaded_game$turn
      game$orders <- interface$loaded_game$orders
      game$hidden_orders <- interface$loaded_game$hidden_orders
      game$difficulty_bias <- interface$loaded_game$difficulty_bias
